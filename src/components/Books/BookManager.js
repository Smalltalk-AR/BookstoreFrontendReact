...
class BookManager extends Component {
  ...
  handleOnAddBook(book) {
    ...
    BookService.addBook(title, editorial, releaseYear, author)
      .then(newBook => {
        BookService.listBooks()
          .then(books => {
            books.forEach(n =>
              n.id === newBook.id ? (n.isNew = 'true') : (n.isNew = undefined)
            )
            this.setState({ books })
          })
    ...
    BookService.updateBook(book)
      .then(() => {
        BookService.listBooks()
          .then(books => {
            books.forEach(n =>
              n.id === book.id ? (n.isNew = 'true') : (n.isNew = undefined)
            )
            this.setState({ books })
          })
          .catch(error => console.log(error))
  ...
  render() {
    return (
      <div>
        <Modal
          isOpen={this.state.isAddBookModalOpen}
          onRequestClose={this.handleOnCloseAddBookModal}
        >
          <AddBookForm
            onSaveBook={this.handleOnAddBook}
            onCloseModal={this.handleOnCloseAddBookModal}
            authors={this.state.authors}
          />
        </Modal>
        <Modal
          isOpen={this.state.isEditBookModalOpen}
          onRequestClose={this.handleOnCloseEditBookModal}
        >
          <EditBookForm
            onSaveBook={this.handleOnEditBook}
            onCloseModal={this.handleOnCloseEditBookModal}
        ...
        <div className="mb-3">
          <BookControlPanel
            openAddBookModal={this.handleOpenAddBookModal}
            onFindBooks={this.handleOnFindBooks}
          />
        </div>
        <BookTable
          books={this.state.books}
          onDeleteBook={this.handleOnDeleteBook}
          onOpenEditBookModal={this.handleOpenEditBookModal}
        />
      </div>
    )
  }
