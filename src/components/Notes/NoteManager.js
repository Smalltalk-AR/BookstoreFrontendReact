...
class NoteManager extends Component {
  ...
  handleOnAddNote(note) {
    ...
    NoteService.addNote(title, content, tags)
      .then(newNote => {
        NoteService.listNotes()
          .then(notes => {
            notes.forEach(n =>
    ...
    return (
      <div>
        <Modal
          ...
          onRequestClose={this.handleOnCloseAddNoteModal}
        >
          <AddNoteForm
            onSaveNote={this.handleOnAddNote}
            onCloseModal={this.handleOnCloseAddNoteModal}
          />
        </Modal>
        <Modal
          isOpen={this.state.isEditNoteModalOpen}
          onRequestClose={this.handleOnCloseEditNoteModal}
        >
          <EditNoteForm
            onSaveNote={this.handleOnEditNote}
            onCloseModal={this.handleOnCloseEditNoteModal}
            note={this.state.selectedNote}
          />
        </Modal>
        <div className="mb-3">
          <ControlPanel
            openAddNoteModal={this.handleOpenAddNoteModal}
            onFindNotes={this.handleOnFindNotes}
          />
        </div>
        <NoteTable
          notes={this.state.notes}
          onDeleteNote={this.handleOnDeleteNote}
