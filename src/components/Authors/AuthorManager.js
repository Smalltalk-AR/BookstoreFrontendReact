...
class AuthorManager extends Component {
  constructor(props) {
    ...
    this.handleOnCloseAddAuthorModal = this.handleOnCloseAddAuthorModal.bind(
      this
    )

    this.handleOpenEditAuthorModal = this.handleOpenEditAuthorModal.bind(this)
    this.handleOnCloseEditAuthorModal = this.handleOnCloseEditAuthorModal.bind(
      this
    )
  }
  ...
  handleOnFindAuthors(firstName) {
    ...
    AuthorService.findAuthorsByFirstName(firstName)
      .then(authors => {
        if (!authors) {
          authors = []
        }
  ...
  handleOnAddAuthor(author) {
    ...
    AuthorService.addAuthor(firstName, lastName, country)
      .then(newAuthor => {
        AuthorService.listAuthors()
          .then(authors => {
            authors.forEach(n =>
              n.id === newAuthor.id ? (n.isNew = 'true') : (n.isNew = undefined)
            )
            this.setState({ authors })
          })
    ...
    AuthorService.updateAuthor(author)
      .then(() => {
        AuthorService.listAuthors()
          .then(authors => {
            authors.forEach(n =>
              n.id === author.id ? (n.isNew = 'true') : (n.isNew = undefined)
            )
            this.setState({ authors })
          })
          .catch(error => console.log(error))
  ...
  render() {
    return (
      <div>
        <Modal
          isOpen={this.state.isAddAuthorModalOpen}
          onRequestClose={this.handleOnCloseAddAuthorModal}
        >
          <AddAuthorForm
            onSaveAuthor={this.handleOnAddAuthor}
            onCloseModal={this.handleOnCloseAddAuthorModal}
          />
        </Modal>
        <Modal
          isOpen={this.state.isEditAuthorModalOpen}
          onRequestClose={this.handleOnCloseEditAuthorModal}
        >
          <EditAuthorForm
            onSaveAuthor={this.handleOnEditAuthor}
            onCloseModal={this.handleOnCloseEditAuthorModal}
            author={this.state.selectedAuthor}
          />
        </Modal>
        <div className="mb-3">
          <AuthorControlPanel
            openAddAuthorModal={this.handleOpenAddAuthorModal}
            onFindAuthors={this.handleOnFindAuthors}
          />
        </div>
        <AuthorTable
          authors={this.state.authors}
          onDeleteAuthor={this.handleOnDeleteAuthor}
          onOpenEditAuthorModal={this.handleOpenEditAuthorModal}
        />
      </div>
    )
  }
